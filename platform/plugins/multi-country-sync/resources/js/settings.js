...
(function() {
    ...
    function init() {
        ...
        // Initialize test connection handler with retry mechanism
        initTestConnection();
    }
    
    // Test Connection handler with retry mechanism
    function initTestConnection(retries = 0) {
        const testConnectionBtn = document.getElementById('test-connection-btn');
        const testConnectionResult = document.getElementById('test-connection-result');
        
        if (!testConnectionBtn) {
            if (retries < 20) {
                console.log(`Test connection button not found, retrying... (${retries + 1}/20)`);
                setTimeout(() => initTestConnection(retries + 1), 200);
            } else {
                console.warn('Test connection button not found after 20 attempts');
            }
            return;
        }
    ...
    // Also use document-level event delegation as fallback
    document.addEventListener('click', function(e) {
        const btn = e.target.closest('#test-connection-btn');
        if (!btn) return;
        
        // Only handle if button doesn't have our direct listener
    ...
    // Also try after a delay in case form is loaded dynamically
